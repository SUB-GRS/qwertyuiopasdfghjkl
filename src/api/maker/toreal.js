...
module.exports = function (app) {
    app.get('/maker/toreal', async (req, res) => {
        const { url } = req.query;
        const fixedPrompt = `[Technical Task: Photorealistic Reconstruction]
1. Subject Analysis: Convert the 2D anime character in the image into a high-fidelity 3D human subject.
2. Texture Mapping: Apply realistic human skin textures, including pores, fine lines, and natural subsurface scattering.
3. Feature Enhancement: Transform stylized anime eyes into anatomically correct human eyes with realistic irises, pupils, and wetness. Replace stylized hair with realistic hair strands, follicles, and natural sheen.
4. Lighting: Apply cinematic volumetric lighting. Ensure global illumination and ambient occlusion match a real-world environment.
5. Detail Level: 8k resolution, raw photo style, highly detailed facial features, realistic clothing fabric textures (cotton/leather/silk), and depth of field.
Eliminate all 2D cel-shading, outlines, and flat colors. Output must be a hyper-realistic photographic representation.`;
