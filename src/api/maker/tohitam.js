...
module.exports = function (app) {
    app.get('/maker/tohitam', async (req, res) => {
        const { url } = req.query;
        const fixedPrompt = `Anda adalah spesialis kolorimetri digital dan ahli dermatologi visual.
Tugas: Analisis foto subjek dan lakukan transformasi warna kulit menjadi gelap natural (Deep/Dark Skin Tone).
Instruksi:Peta Segmentasi Dermas: Identifikasi seluruh area kulit yang terekspos (wajah, leher, dan ekstremitas). Pisahkan mask kulit dari elemen non-kulit seperti sklera mata, gigi, dan pakaian untuk menjaga integritas warna aslinya.Penyesuaian Melanin (Value & Saturation): Turunkan nilai Luminance pada kanal warna kulit secara gradual. Pastikan distribusi warna mengikuti anatomi wajah agar area highlight (tulang pipi, dahi) tetap memiliki dimensi dan tidak terlihat datar/flat.Kalibrasi Undertone: Analisis undertone dasar subjek. Transformasikan menjadi Rich Umber atau Deep Golden untuk menghindari efek ashen (abu-abu). Pastikan transisi warna pada area bayangan memiliki saturasi yang hangat.Preservasi Tekstur (High-Frequency Detail): Pertahankan detail pori-pori, kerutan halus, dan karakteristik kulit asli. Jangan melakukan blurring berlebih pada area yang digelapkan.Simulasi Subsurface Scattering: Sesuaikan pantulan cahaya pada tepian bayangan kulit untuk mensimulasikan kepadatan melanin yang realistis di bawah pencahayaan yang ada.Output: Harus teknis, mendetail, dan menyertakan rekomendasi palet warna (Hex/RGB) untuk target kulit baru agar bisa diproses oleh tim editor/engine manipulasi gambar.`;
