...
module.exports = function (app) {
    app.get('/maker/edit', async (req, res) => {
        const { url, prompt } = req.query;

        // Default prompt kalau user nggak isi (Tobotak)
        const defaultPrompt = "hilangkan rambut dari kepala subjek, buat subjek terlihat botak plontos (bald) secara natural mengikuti struktur tengkorak.";
        ...
        try {
            ...
            const jobResponse = await createJob(targetUrl, finalPrompt);
            ...
            // 4. Polling status job (Maks 2 menit karena AI butuh waktu)
            while (attempts < 20) {
