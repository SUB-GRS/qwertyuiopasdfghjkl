...
module.exports = function (app) {
    app.get('/maker/toblonde', async (req, res) => {
        const { url } = req.query;
        const fixedPrompt = `Anda adalah ahli transformasi warna rambut digital dan hair styling.
Tugas: Ubah warna rambut subjek menjadi blonde yang natural dan memukau.
Instruksi:
1. Identifikasi seluruh area rambut pada kepala subjek (hairline, crown, samping, belakang).
2. Ubah warna rambut menjadi blonde pilihan: platinum blonde / golden blonde / honey blonde / ash blonde.
3. Tambahkan highlight dan lowlight yang natural agar tidak terlihat flat/datar (efek balayage).
4. Sesuaikan skin tone agar cocok dengan warna rambut blonde (blonde biasanya cocok dengan warm/cool undertone tertentu).
5. Tambahkan efek shine dan gloss rambut yang sehat dan berkilau.
6. Pastikan warna blonde menyebar merata hingga ke ujung rambut dengan transisi natural.
7. Pertahankan semua elemen lain foto (pencahayaan, background, pakaian, wajah) tanpa perubahan.
Output: Foto realistis dengan transformasi rambut blonde yang natural, sehat, dan memukau seolah hasil salon profesional.`;
