...
━━━━━━━━━━━━━━━━━━
<i>gootaaaaa API's Log System ✨</i>`;
    await sendTelegram(text);
}
...
// ========== STATIC FILES ==========
app.use("/", express.static(path.join(ROOT_DIR, "gootaaaaa")));
...
let totalRoutes = 0;
const apiFolder = path.join(ROOT_DIR, "src", "qwerty");
...
app.get("/page", (req, res) => res.sendFile(path.join(ROOT_DIR, "gootaaaaa", "index.html")));
app.get("/dashboard", (req, res) => res.sendFile(path.join(ROOT_DIR, "gootaaaaa", "dashboard.html")));
app.get("/ai", (req, res) => res.sendFile(path.join(ROOT_DIR, "gootaaaaa", "ai.html")));

app.use((req, res) => res.status(404).sendFile(path.join(ROOT_DIR, "gootaaaaa", "404.html")));

app.use((err, req, res, next) => {
    console.error(err.stack);
    sendNotification(`🚨 <b>SERVER ERROR</b>\n\n<code>${err.message}</code>`);
    res.status(500).sendFile(path.join(ROOT_DIR, "gootaaaaa", "500.html"));
});
